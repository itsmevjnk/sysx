...
void pmm_init() {
		...
		for(size_t i = 0; entry != NULL; i++) {
			...
			if(!entry->addr_h && !entry->len_h && entry->type == MULTIBOOT_MEMORY_AVAILABLE) {
				...
				for(size_t i = entry->addr_l >> 12; i < (size_t) frame_hi; i++) {
					if(i >= ((kernel_start - 0xC0000000) >> 12)) pmm_free(i); // reserve the memory space before the kernel (i.e. the first 16M) as we may need access to it later
				}
			}
	...
	for(size_t frame = ((kernel_start - 0xC0000000) >> 12); frame < ((kernel_end - 0xC0000000) >> 12); frame++)
		pmm_alloc(frame);
