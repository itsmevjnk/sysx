...
static size_t kheap_expand(size_t size) {
    ...
    for(size_t i = 0; i < requested_frames; i++) {
        ...
        vmm_pgmap(vmm_current, frame * pmm_framesz(), (uintptr_t) kheap_first_block + kheap_size + i * pmm_framesz(), 0, VMM_FLAGS_PRESENT | VMM_FLAGS_RW | VMM_FLAGS_GLOBAL | VMM_FLAGS_CACHE); // map new frame to heap memory space
        alloc_frames++;
    }
