...
void* task_create(bool user, void* src_task, uintptr_t entry) {
    ...
    for(size_t i = 0; i < (rq_stack_size + framesz - 1) / framesz; i++) {
        ...
        stack_frames++;
        pmm_alloc(frame);
        vmm_pgmap(common->vmm, frame * framesz, kernel_start - stack_frames * framesz, true, user, true);
        if(copy_stack) {
            /* copy stack */
            vmm_pgmap(vmm_current, vmm_physaddr(common_src->vmm, common_src->stack_bottom - stack_frames * framesz), (uintptr_t) stack_copy_src, true, false, true);
            vmm_pgmap(vmm_current, frame * framesz, (uintptr_t) stack_copy_dst, true, false, true);
